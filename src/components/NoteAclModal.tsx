...
export function NoteAclModal({ noteId, isOpen, onClose, onUpdate }: NoteAclModalProps) {
  const { Modal, Alert } = useUi();
  const [acls, setAcls] = useState<NoteAcl[]>([]);
  ...
  const noteAclConfig: AclPickerConfig = useMemo(() => ({
    principals: {
      users: true,
      groups: true,
      roles: true,
