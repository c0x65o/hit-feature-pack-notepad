...
export async function GET(request: NextRequest) {
  try {
    ...
    if (note.userId && note.userId !== user.sub) {
      ...
      if (!hasManageAcl) {
        return NextResponse.json(
          { error: 'Forbidden: Insufficient permissions to view ACLs' },
          { status: 403 }
        );
      }
    }
  ...
  try {
    ...
    if (!body.principalType || !body.principalId || !body.permissions) {
      return NextResponse.json(
        {
          error:
            'Missing required fields: principalType, principalId, permissions',
        },
        { status: 400 }
      );
    }
    ...
    if (note.userId && note.userId !== user.sub) {
      ...
      if (!hasManageAcl) {
        return NextResponse.json(
          {
            error:
              'Forbidden: Insufficient permissions to manage ACLs',
          },
          { status: 403 }
        );
      }
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    if (!noteId || !aclId) {
      return NextResponse.json(
        { error: 'Missing note ID or ACL ID' },
        { status: 400 }
      );
    }
    ...
    if (acl.noteId !== noteId) {
      return NextResponse.json(
        { error: 'ACL does not belong to this note' },
        { status: 400 }
      );
    }
