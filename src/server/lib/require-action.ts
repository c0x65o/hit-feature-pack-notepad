...
export async function checkNotepadAction(
  ...
  if (!token) {
    console.log(`[Notepad Action Check] ${actionKey}: No token found`);
  ...
  console.log(`[Notepad Action Check] ${actionKey}: Checking via ${url}`);
  ...
  const res = await fetch(
    ...
    console.log(`[Notepad Action Check] ${actionKey}: Fetch error:`, e);
    return null;
  });

  if (!res) {
    console.log(`[Notepad Action Check] ${actionKey}: Auth unreachable (no response)`);
    return { ok: false, source: 'auth_unreachable' };
  }
  
  if (!res.ok) {
    const errorText = await res.text().catch(() => '');
    console.log(`[Notepad Action Check] ${actionKey}: Auth returned status ${res.status}:`, errorText);
    return { ok: false, source: `auth_status_${res.status}` };
  }

  const json = (await res.json().catch(() => null)) as any;
  const ok = Boolean(json?.has_permission ?? json?.hasPermission ?? false);
  console.log(`[Notepad Action Check] ${actionKey}: Result`, { ok, source: json?.source, response: json });
...
export async function requireNotepadAction(
  ...
  const result = await checkNotepadAction(request, actionKey);
